...
var ecdsa = require('../lib/eccrypto/ecdsa')
...
describe('ecdsa', function() {
  describe('deterministicGenerateK', function() {
    fixtures.valid.forEach(function(f) {
      it('for \"' + f.message + '\"', function(done) {
  ...
  describe('recoverPubKey', function() {
    fixtures.valid.forEach(function(f) {
      it('recovers the pubKey for ' + f.d, function(done) {
        var d = BigInteger.fromHex(f.d)
    ...
    describe('with i ∈ {0,1,2,3}', function() {
      ...
      points.forEach(function(expectedHex, i) {
        it('recovers an expected point for i of ' + i, function(done) {
  ...
  describe('sign', function() {
    fixtures.valid.forEach(function(f) {
      it('produces a deterministic signature for \"' + f.message + '\"', function(done) {
        var d = BigInteger.fromHex(f.d)
    ...
    it('should sign with low S value', function(done) {
  ...
  describe('verifyRaw', function() {
    fixtures.valid.forEach(function(f) {
      it('verifies a valid signature for \"' + f.message + '\"', function(done) {
        var d = BigInteger.fromHex(f.d)
    ...
    fixtures.invalid.verifyRaw.forEach(function(f) {
      it('fails to verify with ' + f.description, function(done) {
        var d = BigInteger.fromHex(f.d)
        var e = BigInteger.fromHex(f.e)
        var signature = new ECSignature(
          new BigInteger(f.signature.r),
